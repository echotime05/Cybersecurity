...
$serverExeBase = 'v_server.exe'

function Resolve-ServerExe {
    $candidate = Join-Path $buildDir $serverExeBase
    if (Test-Path -LiteralPath $candidate) {
        return $candidate
    }
    foreach ($configName in @('Debug', 'Release', 'RelWithDebInfo', 'MinSizeRel')) {
        $candidate = Join-Path $buildDir (Join-Path $configName $serverExeBase)
        if (Test-Path -LiteralPath $candidate) {
            return $candidate
        }
    }
    return (Join-Path $buildDir $serverExeBase)
}
...
if (-not $NoBuild) {
    ...
    & cmake --build $buildDir --config Debug --target v_server
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }
}
